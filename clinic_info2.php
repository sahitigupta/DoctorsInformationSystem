...
?>
		...
		$sql1=mysql_query("SELECT * FROM clinic c, location l WHERE c.lid = l.lid AND city = '$city'  and country='$country' and state='$state'")or die("Error1");
		$clinic_count=mysql_num_rows($sql1);
		if($clinic_count==0)
		{
		...
		for($k=0;$k<$clinic_count;$k++)
		{
			...
			$Clid=$clinic['Lid'];
			$cplot=$clinic['plot_no'];
			$cstreet=$clinic['Street'];
			$ccity=$clinic['City'];
			$cState=$clinic['State'];
			$cCountry=$clinic['Country'];
			$cpin=$clinic['Pin'];
			
			
			
		echo "<tr bgcolor='#00FFFF'> <td colspan='4' align='center'  > Clinic Details </td> </tr>";
		...
		echo " <td  bgcolor='#CCCCFF'> <label>Address </label></td> <td bgcolor='#CCFFFF'>#$cplot, $cstreet street, $ccity,<br />
															$cState,$cCountry,$cpin </td></tr>";
	
			
			
			
			
			$sql2=mysql_query("SELECT * FROM doctor d, location l WHERE d.lid = l.lid and Did='$Did' ")or die("Error1");
		$count=mysql_num_rows($sql2);
			for($i=0;$i<$count;$i++)
			{
				$temp=$i+1;
			$namesmp=mysql_fetch_array($sql2);
			$id=$namesmp['Did'];
			$name=$namesmp['Fname'];
			$lname=$namesmp['Lname'];
